...
export default function Link({ href, children, external, asButton = false, noExternalIcon, className = "", ...props }: LinkProps) {
  ...
  if (external) return (
    <a
      href={href}
      className={combine("inline-block dark:text-cake-400 dark:hover:text-cake-500 dark:active:text-cake-600 text-cake-600 hover:text-cake-700 active:text-cake-800", buttonClasses, asButton && sriracha.className, noExternalIcon && "no-icon", className)}
      target="_blank"
      rel="noreferrer"
      {...props}
    >
